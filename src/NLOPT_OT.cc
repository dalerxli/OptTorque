/*---------------------------------------------------------------
 * NLOPT_OT.cc : NLOPT run for OptTorque, derivative-free
...
int main(int argc, char *argv[])
{
  ...
  //---------------------------------------------------------------//
  // --- define bounds 
  double lb[n]; // the lower bound for x (zero)
  for (int j=0;j<n;j++){lb[j]= 0.0;}
  double ub[n]; // the lower bound for x (zero)
  for (int l=0;l<NUML;l++){
    lb[l*5]= 90.0;    // set bounds <90 degrees 
    lb[l*5+1]= 1e10; // set bounds to be below 10^10
    lb[l*5+2]= 1e10; // set bounds to be below 10^10
    lb[l*5+3]= 1e10; // set bounds to be below 10^10
    lb[l*5+4]= 1e10; // set bounds to be below 10^10
  }
