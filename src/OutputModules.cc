...
{ 
  ...
  ///updated for v11 to include Omega in the filename 
  snprintf(PPFileName,100,"%s.%s.%s.pp",GeoFileBase,z2s(SSD->Omega),GetFileBase(MeshFileName));
                  ...
                  bool PlotFlux, char *FileName)
{
  ...
  if (PlotFlux)
   { snprintf(FileNameBuffer, 200, "%s.%s.%sPFT.pp", 
              GetFileBase(G->GeoFileName),z2s(Omega),MethodNames[Method]);
